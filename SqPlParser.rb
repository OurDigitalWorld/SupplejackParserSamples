class SqPLparser < SupplejackCommon::Xml::Base
  ...
  attributes :display_content_partner, default: "Squamish Public Library"
 ...
 reject_if do
   get(:category).find_with(/collection/).present?
 end
  ...
  attribute :creator, xpath: "//field[@name='creator']"# do
 # tag = get(:creator).split(",")
  #compose(tag.select(:last), " ", tag.select(:first))
#end
  
  #attributes :creator, xpath: "//field[@name='creator']"
  
  attributes :subject, xpath: "//field[@name='subject']"
  
  attributes :description, xpath: "//field[@name='description']", mappings: {'"' => ''}, truncate: 170 do
